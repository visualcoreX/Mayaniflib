...
namespace Niflib {
	...
	public:
		...
		/*!
		 * Gets the collision filter byte (colFilter). The first bit sets the LINK
		 * property and controls whether this body is physically linked to others.
		 * The next bit turns collision off. Then, the next bit sets the SCALED
		 * property in Oblivion. The next five bits make up the number of this
		 * part in a linked body list. Used for FNV/Oblivion-era files
		 * (info.version < VER_20_2_0_7); Skyrim-era files use
		 * GetFlagsAndPartNumber() instead - see Read()/Write() for the version
		 * check.
		 * \return The current collision filter byte.
		 */
		NIFLIB_API byte GetColFilter() const;

		/*!
		 * Sets the collision filter byte (colFilter). See GetColFilter() for a
		 * description of the bit layout.
		 * \param[in] value The new collision filter byte.
		 */
		NIFLIB_API void SetColFilter(byte value);

		/*!
		 * Gets the flags-and-part-number byte (flagsAndPartNumber). Used for
		 * Skyrim-era files (info.version >= VER_20_2_0_7) in place of
		 * GetColFilter() - see Read()/Write() for the version check.
		 * \return The current flags-and-part-number byte.
		 */
		NIFLIB_API byte GetFlagsAndPartNumber() const;

		/*!
		 * Sets the flags-and-part-number byte (flagsAndPartNumber). See
		 * GetFlagsAndPartNumber() for when this is used instead of
		 * GetColFilter()/SetColFilter().
		 * \param[in] value The new flags-and-part-number byte.
		 */
		NIFLIB_API void SetFlagsAndPartNumber(byte value);
